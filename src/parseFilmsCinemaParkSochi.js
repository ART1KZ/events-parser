...
const CINEMA_URL =
    ...
    "https://kinoteatr.ru/raspisanie-kinoteatrov/sochi/more-moll/";
...
// Функция для создания ISO строки с часовым поясом +05:00 и добавлением +2 часа
function toISOStringWithOffset(date) {
    // Добавляем 2 часа к времени сеанса для синхронизации с сервером
...
function groupSessionsByMovieAndDay(sessions, daysCount, pageDate) {
    // Начало и конец интервала — от даты страницы
    ...
    log.info(
        `Парсим сеансы на ${daysCount} дней с ${start.toDateString()} по ${end.toDateString()}`
    );

    const groups = new Map();

    for (const session of sessions) {
        // Используем оригинальную дату БЕЗ смещения для группировки
    ...
    for (const [_, movieSessions] of groups) {
        ...
        // Для описания используем оригинальное время БЕЗ +2 часов
        ...
        const mainSession = {
            ...
            allShowTimes: allTimes, // Оригинальное время для показа
        };
...
async function downloadImage(
    ...
    const headers = {
        "User-Agent":
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Crawlee/cheerio",
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            ...
            log.warning(
                `Попытка ${attempt} для ${imageUrl} не удалась: ${e.message}. Жду ${backoffMs} мс`
            );
    ...
    if (!res || !res.ok) {
        throw new Error(
            `Не удалось скачать: ${imageUrl} (${
                lastErr?.message || "нет ответа"
            })`
        );
    }
...
function describeFetchError(e) {
    ...
    return `${e.message} (${code}${addr ? ` ${addr}` : ""}${
        port ? `:${port}` : ""
    })`;
}
...
function extractAllSessions($, baseUrl, pageDate) {
    ...
    $(".shedule_movie.bordered").each((_, movieEl) => {
        ...
        // Название фильма
        const baseTitle =
            movie.find(".movie_card_header.title").first().text().trim() || "";
        if (!baseTitle) return;

        // Возрастной рейтинг
        const ratingText =
            movie.find(".movie_card_raiting.sub_title").text() || "";
        ...
        let coverUrl =
            pickImageUrlFromImg(
                $,
                movie.find(".shedule_movie_img").first(),
                baseUrl
            ) || null;
        ...
        // Сеансы
        movie
            .find(".shedule_movie_sessions a.buy_seance")
            .each((__, seanceEl) => {
                const seance = $(seanceEl);

                const timeText = seance
                    .find(".shedule_session_time")
                    .text()
                    .trim();
                if (!timeText) return;
                ...
                // Оригинальная дата сеанса БЕЗ смещения
                ...
                sessions.push({
                    ...
                    abbtitle,
                    baseTitle,
                    originalDate: sessionDate, // Оригинальное время для описания
                    dateStart: toISOStringWithOffset(sessionDate), // +2 часа для базы
                    dateEnd: null,
                    site,
                    description: "",
                    _coverUrl: coverUrl,
...
async function main() {
    ...
    if (!connectionOk) {
        log.error(
            "Не удается подключиться к сайту кинотеатра. Завершаю работу."
        );
        process.exit(1);
    }
    ...
    const crawler = new CheerioCrawler(
        {
            ...
            async requestHandler({ request, $, log, session }) {
                ...
                log.info(
                    `После группировки и фильтрации: ${groupedItems.length} записей`
                );
                    ...
                    tasks.push(
                        (async () => {
                            try {
                                ...
                                log.warning(
                                    `Ошибка получения описания для ${movieUrl}: ${e.message}`
                                );
