...
async function main() {
  ...
  params.set('filters[id][$gte]', '851');
  params.set('filters[id][$lte]', '976');
  ...
  if (!items.length) {
    console.log('Нет записей в этом диапазоне.');
    return;
  }

  console.log(`Найдено ${items.length} записей, обновляем categories на [28]...`);
  ...
  for (const item of items) {
    ...
    if (!docId) {
      console.error(`❌ ID ${numericId}: documentId не найден`);
      fail++;
      continue;
    }
    ...
    try {
      ...
      console.log(`✅ ID ${numericId} обновлён`);
      success++;
    } catch (e) {
      console.error(`❌ ID ${numericId} -> ${e.message}`);
      fail++;
    }
  }

  console.log(`\nРезультат: успешно ${success}, ошибок ${fail}`);
}
